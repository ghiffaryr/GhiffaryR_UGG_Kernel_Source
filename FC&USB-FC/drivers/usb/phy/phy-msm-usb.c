...
static void msm_otg_notify_charger(struct msm_otg *motg, unsigned mA)
{
	...
	/*
	 * Limit type-c charger current to 900 for SDP charger to avoid more
	 * current drawn than 900 with Hosts that don't support type C due to
	 * non compliant type-c to standard A cables.
	 */
	if (pdata->enable_sdp_typec_current_limit &&
			(motg->chg_type == USB_SDP_CHARGER) &&
					motg->typec_current_max > 900)
		motg->typec_current_max = 900;

	/* Override mA if type-c charger used (use hvdcp/bc1.2 if it is 900) */
	if (motg->typec_current_max > 900 && mA < motg->typec_current_max)
		mA = motg->typec_current_max;
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		/*
		 * Limit type-c charger current to 900 for SDP charger
		 * to avoid more current drawn than 900 with legacy Hosts.
		 */
		if (pdata->enable_sdp_typec_current_limit &&
				(motg->chg_type == USB_SDP_CHARGER)
					&& val->intval > 900)
